...
const STATUS = {
  idle:        { icon: "👁", label: "대기",       color: "#64748b" },
  ...
  unsupported: { icon: "✕",  label: "미지원",     color: "#ef4444" },
};
...
function Widget() {
  ...
  const [pos,     setPos]     = useState({ x: 16, y: 16 }); // 우하단 fixed 기준
  const [open,    setOpen]    = useState(true); // 패널 열림/닫힘
  ...
  const detect = useCallback(async () => {
    ...
    try {
      ...
      if (!faces.length) { setRatio(0); setStatus("noface"); return; }
      const big = faces.reduce((a, b) =>
  ...
  const start = useCallback(async () => {
    if (!("FaceDetector" in window)) { setStatus("unsupported"); return; }
    try {
  ...
  const stop = useCallback(() => {
    ...
    setRunning(false); setStatus("idle"); setRatio(0);
  }, []);
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      setPos(p => ({ x: p.x - dx, y: p.y - dy })); // right/bottom 값이므로 반전
    };
...
// ── Portal 래퍼 (어디서 import해도 body에 렌더) ────────
